  ...
  constructor() {
    super({
      ...
      rule: [
        {
          reg: '^#?(高考|gaokao|倒计时)$',
          fnc: 'getGaokaoCountdown'
        }
      ]
    })
  }

  async getGaokaoCountdown(e) {
    const gaokaoDate = new Date(new Date().getFullYear(), 5, 7, 9, 0, 0) // 高考时间：每年6月7日9:00
    const now = new Date()
    
    // 如果当前时间已过高考时间，则计算下一年的高考
    if (now > gaokaoDate) {
      gaokaoDate.setFullYear(gaokaoDate.getFullYear() + 1)
    }
    
    const timeDiff = gaokaoDate - now
    ...
    const msg = `高考还剩：${days}天${hours}小时${minutes}分${seconds}秒\n高考时间：${gaokaoDate.getFullYear()}年6月7日 9:00:00\n高考一言：${randomQuote}`
